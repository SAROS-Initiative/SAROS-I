  ...
  */
  ...
  uint8_t &getData(const uint8_t addr, T &value) const {
    ...
    */
    ...
    {                                               //
      Wire1.beginTransmission(_I2CAddress);          // Address the I2C device
      Wire1.write(addr);                             // Send register address to read
      Wire1.endTransmission();                       // Close transmission
      Wire1.requestFrom(_I2CAddress, sizeof(T));     // Request 1 byte of data
      structSize = Wire1.available();                // Use the actual number of bytes
      for (uint8_t i = 0; i < structSize; i++)
        *bytePtr++ = Wire1.read();  // loop for each byte to be read
    }                              //
    else                           //
    {                              //
    ...
    */
    ...
    {                                                     //
      ...
      for (uint8_t i = 0; i < sizeof(T); i++)
        Wire1.write(*bytePtr++);  // loop for each byte to be written
      Wire1.endTransmission();    // Close transmission
    } else {
      if (_sck == 0)  // if sck is zero then hardware SPI
      {
